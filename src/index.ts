...
class State extends ListenerState<Item> {
  ...
  changeState() {
    this.listItems = this.listItems.map((item) => {
      if (item.selected) {
        if (item.state === "available") item.state = "picked";
        else item.state = "available";
  ...
  implements Draggable
{
  ...
  configure() {
    ...
    this.element.addEventListener("click", () => {
      ...
      //   console.log("ici", this.item.selected);
  ...
  implements DragTarget
{
  ...
  contentRender() {
    ...
    this.element.querySelector(
      "h2"
    )!.innerText = `${this.type.toUpperCase()} ITEMS`;
  }

  private itemsRender() {
    const listEl = <HTMLUListElement>(
      document.getElementById(`${this.type}-items-list`)
    );
    listEl.innerHTML = "";
